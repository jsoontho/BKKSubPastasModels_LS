...
for n_ens in range(n):
    ...
    for temp_i in range(len(temp)):
        ...
        if "constant_d" in variable_name:
            ...
            temp_val = temp[temp_i].split("+/- ")[0].strip()
            ls_sub_m[variable_name].append(float(temp_val[10:]))
...
def generate_pumping_ens(ann_pump, n):
...
for well_i, wells in enumerate(well_names):
    ...
    if "pump" in esmdaflag:
        mean_ = generate_pumping_ens(annual_pump, 1)
...
if "pump" in esmdaflag:
    ...
    comppump = generate_pumping_ens(listdaily_pump, 1)
